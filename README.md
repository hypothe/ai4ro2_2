...
ai4ro2_2/
    ...
    visit_module/   , motion planning files
        ...
        src/            , motion planning scripts
            ...
            VisitSolver.cpp     , definition of the class implementing the interface, computing the EKF
            VisitSolver.h       , declaration of the class implementing the interface, computing the EKF
            buildInstruction.txt, script used to build the library
            main.cpp            , main using the solver, can be used for a standalone test executable (unused here)
    |
...
The planner for which the system is defined is [popf-tif](https://github.com/popftif/popf-tif), chosen thanks to its ability to deal with numerical fluents, temporal planning and *semantic attachments*.
...
To simulate the robot movement, the path is then split up into segment which length is the distance traveled (ideally) by the robot between two consecutive odometry measurements (see later subsection with estimations for more info on this aspect).
...
....o...o....  
```
This is the map described by the files currently present in this repository, where the `*` represent waypoints, `o` beacons, and the lines edges of the region graph (`-`, `|`, and also `x`, representing two diagonal connections).
